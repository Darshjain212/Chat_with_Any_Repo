...
def process(
    ...
    co = cohere.Client('Input-Your-Coherent-Key')
    ...
    for text in texts:
        ...
        embeddings.append(response.generations[0].text.strip())

    db = DeepLake(dataset_path=activeloop_dataset_path,
                  embedding_function=None)
    for i, embedding in enumerate(embeddings):
        db.add_document(texts[i], embedding=embedding)
